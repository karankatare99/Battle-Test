...
# -------------------------------
# BATTLE SYSTEM WITH FIXED ANIMATION AND MESSAGE ORDER
# -------------------------------
...
def calculate_damage(attacker, defender, move_key):
    ...
    FIXED: Effectiveness comes first, then critical hit
    """
    ...
    # FIXED: Build text with correct order - effectiveness first, then crit
    text = f"{attacker['name']} used {move.get('Name', move_key)}!\n"
    if phrase:  # Effectiveness message first
        text += f"{phrase}\n"
    if crit > 1.0:  # Critical hit after effectiveness
        text += "A critical hit!\n"
...
async def send_battle_ui(bid, battle_texts=None, disable_buttons=False):
    """
    UPDATED: Added disable_buttons parameter to remove buttons during animation
    """
    battle = battle_storage.get(bid)
    ...
    # Build challenger buttons (or empty if disabled)
    c_buttons = []
    if not disable_buttons:  # Only show buttons if not disabled
...
async def animate_battle_sequence(bid, actions_results):
    """
    UPDATED: Animate the battle sequence step by step with buttons disabled during animation.
    """
    battle = battle_storage.get(bid)
    if not battle:
        return
    
    # Process each action with animation and disabled buttons
    for i, (text_challenger, text_opponent, hp_changes) in enumerate(actions_results):
        # Show action text with buttons disabled
        battle_texts = {
            "challenger": text_challenger,
            "opponent": text_opponent
        }
        await send_battle_ui(bid, battle_texts, disable_buttons=True)
        ...
        # Update UI with HP changes, still with buttons disabled
        await send_battle_ui(bid, battle_texts, disable_buttons=True)
...
async def resolve_turn(bid):
    ...
    # Refresh battle UI for next turn with buttons enabled
    await send_battle_ui(bid, disable_buttons=False)
