...
async def search_for_opp_trainer(lobby):
    ...
    while True:
        ...
        if len(lobby) >= 2:
            ...
            await asyncio.sleep(1)
            p1_msg=await search_msg[p1].edit(f"A battle against {p2} is about to start")
            p2_msg=await search_msg[p2].edit(f"A battle against {p1} is about to start")
...
async def first_battle_ui(mode,fmt,user_id, event):
    if fmt=="singles":
        ...
        print("room", room) 
        p1_poke_hpbar = await hp_bar(battle_data[p1_id]["pokemon"][p1_poke]["current_hp"], battle_data[p1_id]["pokemon"][p1_poke]["stats"]["hp"]) 
        p2_poke_hpbar = await hp_bar(battle_data[p2_id]["pokemon"][p2_poke]["current_hp"], battle_data[p2_id]["pokemon"][p2_poke]["stats"]["hp"])
        p1hppercent=battle_data[p1_id]["pokemon"][p1_poke]["current_hp"]/battle_data[p1_id]["pokemon"][p1_poke]["stats"]["hp"]*100
        p2hppercent=battle_data[p2_id]["pokemon"][p2_poke]["current_hp"]/battle_data[p2_id]["pokemon"][p2_poke]["stats"]["hp"]*100
        p1_text= (
...
async def move_data_extract(move):
    with open("moves.json","r") as f:
        ...
        category = move_data[move]["Category"]
        power = move_data[move]["power"]
        acc = move_data[move]["Accuracy"]
        return type, category,power,acc
async def type_modifier(move_type,opp_type1,opp_type2):
    type1_modifier[move_type][opp_type1]
    type2_mofifier[move_type][opp_type2] if opp_type2 is not None else 1
...
async def select_pokemon(event):
    ...
    # decode poke if bytes
    if isinstance(poke, bytes):
        poke = poke.decode()

    current_team = select_team[user_id]["pokes"]
