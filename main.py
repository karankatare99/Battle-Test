...
# BATTLE SYSTEM WITH SINGLES AND DOUBLES SUPPORT
...
def load_battle_pokemon(bid):
    ...
    if battle["type"] == "singles":
        ...
        if len(battle["battle_state"]["challenger"]) >= 2 and len(battle["battle_state"]["opponent"]) >= 2:
            ...
            return False # Not enough Pokémon for doubles
...
def create_battle(challenger_id, opponent_id, battle_type):
    ...
    battle_storage[bid] = {
        ...
        "pending_action": {"challenger": {}, "opponent": {}}, # Now stores multiple actions

        "forced_switch": {"challenger": [], "opponent": []}, # Now tracks which slots need switches
...
def build_perspective_texts(battle, attacker_side, attacker_slot, defender_side, defender_slot, base_text, attacker_name, defender_name):

    """

    Build perspective-aware texts for both players in singles/doubles.
    ...
    if battle["type"] == "singles":
        ...
        challenger_name = battle["active"]["challenger"][0]["name"]

        opponent_name = battle["active"]["opponent"][0]["name"]
        ...
        # In doubles, we need slot-specific naming

        challenger_names = [p["name"] for p in battle["active"]["challenger"]]

        opponent_names = [p["name"] for p in battle["active"]["opponent"]]

        if attacker_side == "challenger":

            # Challenger's Pokémon attacks

            attacker_display_name = challenger_names[attacker_slot]

            if defender_side == "challenger":

                defender_display_name = challenger_names[defender_slot]

            else:

                defender_display_name = opponent_names[defender_slot]

            # For challenger view

            chal_attacker = attacker_display_name

            chal_defender = f"Opposing {defender_display_name}" if defender_side == "opponent" else defender_display_name

            # For opponent view

            opp_attacker = f"Opposing {attacker_display_name}"

            opp_defender = defender_display_name if defender_side == "opponent" else f"Opposing {defender_display_name}"

        else:

            # Opponent's Pokémon attacks

            attacker_display_name = opponent_names[attacker_slot]
            ...
            chal_attacker = f"Opposing {attacker_display_name}"

            chal_defender = defender_display_name if defender_side == "challenger" else f"Opposing {defender_display_name}"

            # For opponent view

            opp_attacker = attacker_display_name

            opp_defender = f"Opposing {defender_display_name}" if defender_side == "challenger" else defender_display_name
...
# Fixed send_battle_ui function that handles action selection order properly
async def send_battle_ui(bid, battle_texts=None, buttons=None):
    ...
    # Helper to check if any active Pokémon are fainted
    def get_fainted_slots(active_list):
        return [i for i, pkm in enumerate(active_list) if pkm["hp"] <= 0]

    challenger_active = battle["active"]["challenger"]  
    opponent_active = battle["active"]["opponent"]

    challenger_fainted = get_fainted_slots(challenger_active)
    opponent_fainted = get_fainted_slots(opponent_active)

    # Build challenger UI
    c_text = f"⚔️ Battle Turn {battle.get('turn', 1)} - {battle['type'].title()}\n\n"

    # Show challenger's Pokémon
    c_text += "Your Pokémon:\n"
    for i, pkm in enumerate(challenger_active):
        status = " 💀" if pkm["hp"] <= 0 else ""
        c_text += f" {i+1}. {pkm['name']}: {pkm['hp']}/{pkm['max_hp']} HP{status}\n"
        c_text += f" {hp_bar(pkm['hp'], pkm['max_hp'])}\n"

    c_text += "\nOpposing Pokémon:\n"
    for i, pkm in enumerate(opponent_active):
        status = " 💀" if pkm["hp"] <= 0 else ""
        c_text += f" {i+1}. Opposing {pkm['name']}: {pkm['hp']}/{pkm['max_hp']} HP{status}\n"
        c_text += f" {hp_bar(pkm['hp'], pkm['max_hp'])}\n"
    ...
    # Check if forced switches are needed
    if challenger_fainted or battle["forced_switch"]["challenger"]:
        c_text += "\n\n💀 Choose replacement Pokémon:"
        team = battle["battle_state"]["challenger"]

        # Determine which slots need replacement
        slots_to_replace = challenger_fainted if challenger_fainted else battle["forced_switch"]["challenger"]

        for slot in slots_to_replace:
            c_text += f"\n\nSlot {slot + 1} replacement:"
            slot_buttons = []
            for idx, pkm in enumerate(team):
                # Can't switch to active Pokémon or fainted ones
                if (pkm["hp"] > 0 and
                    pkm["id"] not in [active["id"] for active in challenger_active]):
                    slot_buttons.append([Button.inline(
                        f"{pkm['name']} ({pkm['hp']}/{pkm['max_hp']} HP)",
                        f"battle:forced_switch:{bid}:challenger:{slot}:{idx}"
                    )])
            c_buttons.extend(slot_buttons)
            break  # Handle one slot at a time
        ...
        if battle["type"] == "singles":
            # Singles: simple move selection
            for move in challenger_active[0].get("moves", []):
            ...
            # FIXED: Better handling of doubles action selection
            challenger_actions = battle["pending_action"]["challenger"]

            # Count how many actions we still need
            actions_needed = []
            for slot in range(len(challenger_active)):
                if (slot not in challenger_actions and 
                    challenger_active[slot]["hp"] > 0):
                    actions_needed.append(slot)

            if actions_needed:
                # Show actions for next available slot
                next_slot = actions_needed[0]
                c_text += f"\n\nChoose action for {challenger_active[next_slot]['name']} (Slot {next_slot+1}):"

                # Move buttons with target selection needed
                for move in challenger_active[next_slot].get("moves", []):
                    c_buttons.append([Button.inline(f"🎯 {move} (choose target)", f"battle:move_select:{bid}:challenger:{next_slot}:{move}")])

                # Switch button
                c_buttons.append([Button.inline("🔄 Switch", f"battle:switch:{bid}:challenger:{next_slot}")])

                # Show status of selected actions
                selected_count = len(challenger_actions)
                total_alive = len([p for p in challenger_active if p["hp"] > 0])
                if selected_count > 0:
                    c_text += f"\n\n✅ Actions selected: {selected_count}/{total_alive}"
                    for slot, action_data in challenger_actions.items():
                        action_name = action_data.get("action", "Unknown")
                        if action_name.startswith("switch:"):
                            action_name = "Switch"
                        c_text += f"\n  • Slot {slot+1}: {action_name}"
            else:
                # All actions selected, waiting for opponent
                c_text += "\n\n⏳ All actions selected! Waiting for opponent..."
                c_buttons.append([Button.inline("🏳️ Forfeit", f"battle:forfeit:{bid}")])

        # Always add forfeit option if no other buttons
        if not c_buttons:
            c_buttons.append([Button.inline("🏳️ Forfeit", f"battle:forfeit:{bid}")])

    # Build opponent UI (similar structure but mirrored)
    o_text = f"⚔️ Battle Turn {battle.get('turn', 1)} - {battle['type'].title()}\n\n"

    # Show opponent's Pokémon  
    o_text += "Your Pokémon:\n"
    for i, pkm in enumerate(opponent_active):
        status = " 💀" if pkm["hp"] <= 0 else ""
        o_text += f" {i+1}. {pkm['name']}: {pkm['hp']}/{pkm['max_hp']} HP{status}\n"
        o_text += f" {hp_bar(pkm['hp'], pkm['max_hp'])}\n"

    o_text += "\nOpposing Pokémon:\n"
    for i, pkm in enumerate(challenger_active):
        status = " 💀" if pkm["hp"] <= 0 else ""
        o_text += f" {i+1}. Opposing {pkm['name']}: {pkm['hp']}/{pkm['max_hp']} HP{status}\n"
        o_text += f" {hp_bar(pkm['hp'], pkm['max_hp'])}\n"
    ...
    # Check if forced switches are needed for opponent
    if opponent_fainted or battle["forced_switch"]["opponent"]:
        o_text += "\n\n💀 Choose replacement Pokémon:"
        team = battle["battle_state"]["opponent"]

        # Determine which slots need replacement  
        slots_to_replace = opponent_fainted if opponent_fainted else battle["forced_switch"]["opponent"]
        ...
        for slot in slots_to_replace:
            ...
            for idx, pkm in enumerate(team):
                # Can't switch to active Pokémon or fainted ones
                if (pkm["hp"] > 0 and
                    pkm["id"] not in [active["id"] for active in opponent_active]):
                    o_buttons.append([Button.inline(
                        f"{pkm['name']} ({pkm['hp']}/{pkm['max_hp']} HP)",  
                        f"battle:forced_switch:{bid}:opponent:{slot}:{idx}"
                    )])
            break  # Handle one slot at a time
        ...
        # Normal battle actions for opponent
        if battle["type"] == "singles":
            # Singles: simple move selection
            for move in opponent_active[0].get("moves", []):
            ...
            # FIXED: Better handling of doubles action selection for opponent
            opponent_actions = battle["pending_action"]["opponent"]

            # Count how many actions we still need
            actions_needed = []
            for slot in range(len(opponent_active)):
                if (slot not in opponent_actions and 
                    opponent_active[slot]["hp"] > 0):
                    actions_needed.append(slot)

            if actions_needed:
                # Show actions for next available slot
                next_slot = actions_needed[0]
                o_text += f"\n\nChoose action for {opponent_active[next_slot]['name']} (Slot {next_slot+1}):"

                # Move buttons with target selection needed
                for move in opponent_active[next_slot].get("moves", []):
                    o_buttons.append([Button.inline(f"🎯 {move} (choose target)", f"battle:move_select:{bid}:opponent:{next_slot}:{move}")])

                # Switch button
                o_buttons.append([Button.inline("🔄 Switch", f"battle:switch:{bid}:opponent:{next_slot}")])

                # Show status of selected actions
                selected_count = len(opponent_actions)
                total_alive = len([p for p in opponent_active if p["hp"] > 0])
                if selected_count > 0:
                    o_text += f"\n\n✅ Actions selected: {selected_count}/{total_alive}"
                    for slot, action_data in opponent_actions.items():
                        action_name = action_data.get("action", "Unknown")
                        if action_name.startswith("switch:"):
                            action_name = "Switch"
                        o_text += f"\n  • Slot {slot+1}: {action_name}"
            else:
                # All actions selected, waiting for challenger
                o_text += "\n\n⏳ All actions selected! Waiting for opponent..."
    ...
    # IMPROVED: Better error handling for message editing
    try:
        # Edit existing messages when possible to avoid spam
        if battle.get("message_ids", {}).get("challenger") and buttons != False:
            try:
                if buttons == True:
                ...
                print(f"Failed to edit challenger message: {edit_error}")
                # Send new message if edit fails
                if buttons == True:
                    msg = await bot.send_message(battle["challenger"], c_text, buttons=c_buttons)
                    battle["message_ids"]["challenger"] = msg.id
        elif buttons == True and battle.get('turn', 1) == 1:
            # Send initial message
            msg = await bot.send_message(battle["challenger"], c_text, buttons=c_buttons)
            battle["message_ids"]["challenger"] = msg.id
        elif buttons == False:
            # Send text-only message for animations
            await bot.send_message(battle["challenger"], c_text)

        # Same for opponent
        if battle.get("message_ids", {}).get("opponent") and buttons != False:
            try:
                if buttons == True:
                ...
                print(f"Failed to edit opponent message: {edit_error}")
                # Send new message if edit fails
                if buttons == True:
            ...
            # Send text-only message for animations
            await bot.send_message(battle["opponent"], o_text)

    except Exception as e:
        print(f"UI update failed: {e}")
        # Fallback: try to send basic messages
        try:
            await bot.send_message(battle["challenger"], f"⚠️ Battle UI update failed. Turn {battle.get('turn', 1)}")
            await bot.send_message(battle["opponent"], f"⚠️ Battle UI update failed. Turn {battle.get('turn', 1)}")
        except:
            print("Complete UI failure - battle may need to be restarted")
...
async def animate_battle_sequence(bid, actions_results):
    ...
    for i, (text_challenger, text_opponent, hp_changes) in enumerate(actions_results):
        ...
        if hp_changes:
            for side_slot, damage in hp_changes.items():
                if damage > 0:
                    side, slot = side_slot.split("_")
                    slot = int(slot)
                    if slot < len(battle["active"][side]):
                        battle["active"][side][slot]["hp"] = max(0, battle["active"][side][slot]["hp"] - damage)
...
async def process_action(battle, side, slot, other_side, target_slot, action):
    ...
    if action.startswith("switch:"):
        ...
        try:
            ...
            battle["active"][side][slot] = new_pkm
        ...
        # Regular move
        try:
            attacker = battle["active"][side][slot] 
            defender = battle["active"][other_side][target_slot]
            ...
            if defender["hp"] <= 0:
                base_text = f"{attacker['name']} tried to attack the fainted {defender['name']}!"
                return base_text, base_text, {}
            ...
            text_challenger, text_opponent = build_perspective_texts(
                battle, side, slot, other_side, target_slot, base_text,
                attacker["name"], defender["name"]
            )

            # Return damage info for HP changes (using side_slot format)
            hp_changes = {f"{other_side}_{target_slot}": dmg} if dmg > 0 else {}

            return text_challenger, text_opponent, hp_changes

        except Exception as e:
            print(f"Move processing error: {e}")
            base_text = f"{battle['active'][side][slot]['name']} used {action}!\nBut something went wrong!"
            return base_text, base_text, {}

# FIXED: Better turn resolution that handles any action order
async def resolve_turn(bid):
    ...
    all_actions = []  # List of (side, slot, action, target_slot, speed)

    for side in ["challenger", "opponent"]:
        for slot, action_data in battle["pending_action"][side].items():
            if isinstance(action_data, dict) and "action" in action_data:
                action = action_data["action"]
                target_slot = action_data.get("target_slot", 0)

                # Get speed for move order
                if action.startswith("switch:"):
                    speed = 999999  # Switches go first
                else:
                    speed = battle["active"][side][slot].get("spe", 0)

                all_actions.append((side, slot, action, target_slot, speed))

    # Sort by speed (highest first, switches first)
    all_actions.sort(key=lambda x: x[4], reverse=True)

    actions_results = []

    # Process each action in speed order
    for side, slot, action, target_slot, speed in all_actions:
        # Determine the target side
        if battle["type"] == "singles":
            other_side = "opponent" if side == "challenger" else "challenger"
            actual_target_slot = 0
        else:  # doubles
            # In doubles, target_slot from action_data determines the target
            action_data = battle["pending_action"][side][slot]
            target_side = action_data.get("target_side")
            if target_side:
                other_side = target_side
            else:
                other_side = "opponent" if side == "challenger" else "challenger"
            actual_target_slot = target_slot

        # Skip if attacker fainted
        if battle["active"][side][slot]["hp"] <= 0:
            continue

        # Skip if target fainted (for attacks)
        if (not action.startswith("switch:") and
            battle["active"][other_side][actual_target_slot]["hp"] <= 0):
            continue

        text_chal, text_opp, hp_changes = await process_action(
            battle, side, slot, other_side, actual_target_slot, action
        )

        actions_results.append((text_chal, text_opp, hp_changes))

        # Check for faints after this action
        if hp_changes:
            for side_slot, damage in hp_changes.items():
                target_side, target_slot_str = side_slot.split("_")
                target_slot_int = int(target_slot_str)
                current_hp = battle["active"][target_side][target_slot_int]["hp"]
                if current_hp - damage <= 0:
                    # Mark this slot for forced switch
                    if target_slot_int not in battle["forced_switch"][target_side]:
                        battle["forced_switch"][target_side].append(target_slot_int)
...
# -------------------------------
# Forced switch handler (updated for slots)
# -------------------------------
...
async def cb_forced_switch(event):
    ...
    slot = int(event.pattern_match.group(3).decode())
    ...
    battle["active"][side][slot] = target_pokemon

    # Remove this slot from forced switch list
    if slot in battle["forced_switch"][side]:
        battle["forced_switch"][side].remove(slot)
...
# -------------------------------
# Move selection (updated for singles/doubles)  
# -------------------------------

@bot.on(events.CallbackQuery(pattern=b"battle:move:(.+):(.+):(.+):(.+)"))
async def cb_move(event):
    # For singles: battle:move:bid:slot:move:target_slot
    bid = event.pattern_match.group(1).decode()
    slot = int(event.pattern_match.group(2).decode())
    move_selected = event.pattern_match.group(3).decode()
    target_slot = int(event.pattern_match.group(4).decode())
    ...
    active_pokemon = battle["active"][side][slot]
    if move_selected not in active_pokemon.get("moves", []):
        return await event.answer("❌ Your Pokémon doesn't know that move!", alert=True)

    # Store the action
    battle["pending_action"][side][slot] = {
        "action": move_selected,
        "target_slot": target_slot
    }

    await event.edit("✅ Move selected! Waiting for other actions...")

    # Check if all required actions are complete
    if battle["type"] == "singles":
        # Singles: need 1 action from each side
        challenger_ready = 0 in battle["pending_action"]["challenger"]
        opponent_ready = 0 in battle["pending_action"]["opponent"]

        if challenger_ready and opponent_ready:
            await resolve_turn(bid)
    else:
        # Doubles: need actions from all alive Pokémon
        challenger_actions = len(battle["pending_action"]["challenger"])
        opponent_actions = len(battle["pending_action"]["opponent"])

        challenger_alive = sum(1 for p in battle["active"]["challenger"] if p["hp"] > 0)
        opponent_alive = sum(1 for p in battle["active"]["opponent"] if p["hp"] > 0)

        if challenger_actions >= challenger_alive and opponent_actions >= opponent_alive:
            await resolve_turn(bid)
        else:
            # Update UI to show next action needed
            await send_battle_ui(bid, buttons=True)

# -------------------------------
# Move selection for doubles (with target selection)
# -------------------------------

@bot.on(events.CallbackQuery(pattern=b"battle:move_select:(.+):(.+):(.+):(.+)"))
async def cb_move_select(event):
    # For doubles or singles: battle:move_select:bid:side:slot:move
    bid = event.pattern_match.group(1).decode()
    side = event.pattern_match.group(2).decode()
    slot = int(event.pattern_match.group(3).decode())
    move_selected = event.pattern_match.group(4).decode()
    ...
    expected_side = "challenger" if user_id == battle["challenger"] else "opponent"
    if side != expected_side:
        return await event.answer("❌ Invalid move request.", alert=True)

    # Build target selection list
    text = f"🎯 Choose target for {move_selected}:"
    buttons = []

    # Opposing side targets
    other_side = "opponent" if side == "challenger" else "challenger"
    for i, target in enumerate(battle["active"][other_side]):
        if target["hp"] > 0:
            # URL encode move name to handle spaces
            import urllib.parse
            move_encoded = urllib.parse.quote(move_selected, safe='')
            buttons.append([Button.inline(
                f"Target {i+1}: {target['name']} ({target['hp']}/{target['max_hp']} HP)",
                f"battle:move_target:{bid}:{side}:{slot}:{move_encoded}:{i}"
            )])

    # Always include self and ally options when alive
    text += "\n\nOr target self/ally:"
    # Self
    self_pkm = battle["active"][side][slot]
    if self_pkm["hp"] > 0:
        import urllib.parse
        move_encoded = urllib.parse.quote(move_selected, safe='')
        buttons.append([Button.inline(
            f"Self: {self_pkm['name']} ({self_pkm['hp']}/{self_pkm['max_hp']} HP)",
            f"battle:move_target:{bid}:{side}:{slot}:{move_encoded}:{slot}:ally"
        )])
    # Ally (if exists and not same slot)
    for i, ally in enumerate(battle["active"][side]):
        if i == slot:
            continue
        if ally["hp"] > 0:
            import urllib.parse
            move_encoded = urllib.parse.quote(move_selected, safe='')
            buttons.append([Button.inline(
                f"Ally {i+1}: {ally['name']} ({ally['hp']}/{ally['max_hp']} HP)",
                f"battle:move_target:{bid}:{side}:{slot}:{move_encoded}:{i}:ally"
            )])

    await event.edit(text, buttons=buttons)
@bot.on(events.CallbackQuery(pattern=b"battle:move_target:(.+):(.+):(.+):(.+):(.+)(?::(.+))?"))
async def cb_move_target(event):
    # battle:move_target:bid:side:slot:move_encoded:target_slot[:ally]
    bid = event.pattern_match.group(1).decode()
    side = event.pattern_match.group(2).decode()
    slot = int(event.pattern_match.group(3).decode())
    move_encoded = event.pattern_match.group(4).decode()
    target_slot = int(event.pattern_match.group(5).decode())
    is_ally = event.pattern_match.group(6) and event.pattern_match.group(6).decode() == "ally"

    # URL decode the move name
    import urllib.parse
    move_selected = urllib.parse.unquote(move_encoded)

    battle = battle_storage.get(bid)
    if not battle:
        return await event.answer("❌ Battle not found.", alert=True)

    # Store the action with target info
    battle["pending_action"][side][slot] = {
        "action": move_selected,
        "target_slot": target_slot,
        "target_side": side if is_ally else ("opponent" if side == "challenger" else "challenger")
    }

    await event.edit(f"✅ {move_selected} targeting slot {target_slot + 1}!")
...
# -------------------------------
# Switch handlers (updated for slots)
# -------------------------------
...
async def cb_switch(event):
    ...
    for idx, pkm in enumerate(team):
        if pkm["hp"] > 0 and pkm["id"] not in active_ids:
            buttons.append([Button.inline(
                f"{pkm['name']} ({pkm['hp']}/{pkm['max_hp']} HP)",
                f"battle:choose_switch:{bid}:{side}:{slot}:{idx}"
            )])

    if not buttons:
        return await event.answer("⚠️ No healthy Pokémon to switch to.", alert=True)
...
async def cb_choose_switch(event):
    ...
    battle["pending_action"][side][slot] = {
        "action": f"switch:{idx}",
        "target_slot": 0  # Not relevant for switches
    }

    await event.edit(f"🔄 Switch to {target_pokemon['name']} registered! Waiting for other actions...")
