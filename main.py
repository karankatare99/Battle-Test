...
def parse_showdown_set(text):
    ...
    for line in lines[1:]:
        ...
        if line.startswith("Ability:"):
            ...
            # Showdown uses e.g., "Jolly Nature"
...
# Reference formulas Gen III+:
# HP: floor(((2B + I + floor(E/4)) * L)/100) + L + 10
# Other: floor((floor(((2B + I + floor(E/4)) * L)/100) + 5) * N)
# Shedinja HP forced to 1. [Bulbapedia/PokéBase]
# Sources: Bulbapedia Stat, PokeBase HP formula, nature 10% up/down. [2][6][10]

# Minimal base stat stub. Replace with full dex or load from DB.
BASE_STATS = {
    # name lowercased without form; extend as needed
...
def normalize_species(name: str):
    # Basic normalization; adapt for forms as needed
...
def calc_stats(species_name: str, level: int, evs: dict, ivs: dict, nature_name: str):
    ...
    return {
        ...
        "stat_spd": stat_spd,
        "stat_spe": stat_spe,
        "total_stats_sum": stat_hp + stat_atk + stat_def + stat_spa + stat_spd + stat_spe,
    }
# ==== END NEW ================================================================
...
async def handle_pokemon_set(event):
    ...
    if user_id in awaiting_pokemon and any(k in text for k in ["Ability:", "EVs:", "Nature", "- "]):
        ...
        # NEW: compute calculated stats and attach before saving
        ...
        # NEW: show calculated totals if present
        if calc:
            msg += "\n\n🧮 Stats: "
...
async def send_remove_page(event, user, page=0):
    ...
    text = f"➖ Select a Pokémon to Remove (Pag
