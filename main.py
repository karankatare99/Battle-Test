...
async def send_battle_ui(bid, battle_texts=None, buttons=None):
    ...
    # Initialize message_ids if not present
    if "message_ids" not in battle:
        battle["message_ids"] = {"challenger": None, "opponent": None}

    # FIXED MESSAGE HANDLING - Always try to edit first, then send new if needed
    try:
        # Handle challenger message
        if buttons == True:  # Only edit when showing buttons
            if battle["message_ids"]["challenger"]:
                ...
                try:
                    ...
                    print(f"Edit failed for challenger, sending new: {edit_error}")
                    # Edit failed, send new message and update ID
            ...
            # Send text-only message for animations (don't edit, always send new)
        ...
        if buttons == True:  # Only edit when showing buttons
            if battle["message_ids"]["opponent"]:
                ...
                try:
                    ...
                    print(f"Edit failed for opponent, sending new: {edit_error}")
                    # Edit failed, send new message and update ID
        ...
        print(f"Complete UI update failed: {e}")
        # Ultimate fallback: try to send basic messages
        try:
