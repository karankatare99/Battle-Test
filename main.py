...
# -------------------------------
# BATTLE SYSTEM WITH SINGLES AND DOUBLES SUPPORT - FIXED MESSAGE EDITING
# -------------------------------
...
# FIXED: Proper message editing logic
async def send_battle_ui(bid, battle_texts=None, buttons=None):
    ...
    # FIXED: Improved message editing and sending logic
    async def send_or_edit_message(user_id, text, buttons_list, is_challenger=True):
        """Helper function to send or edit messages with proper fallback logic."""
        side = "challenger" if is_challenger else "opponent"
        message_id = battle.get("message_ids", {}).get(side)

        # Try to edit existing message first
        if message_id and buttons is not False:
            try:
                if buttons == True and buttons_list:
                    await bot.edit_message(user_id, message_id, text, buttons=buttons_list)
                    return True
                elif buttons == False:
                    await bot.edit_message(user_id, message_id, text)
                    return True
            except Exception as edit_error:
                print(f"Message edit failed for {side}: {edit_error}")
                # Clear invalid message ID
                battle["message_ids"][side] = None

        # Send new message if edit failed or no message ID
        try:
            if buttons == True and buttons_list:
                msg = await bot.send_message(user_id, text, buttons=buttons_list)
                battle["message_ids"][side] = msg.id
                return True
            elif buttons == False:
                # For animations, just send without storing ID
                await bot.send_message(user_id, text)
                return True
        except Exception as send_error:
            print(f"Message send failed for {side}: {send_error}")
            return False

        return False

    # Send/edit messages for both players
    try:
        success_challenger = await send_or_edit_message(
            battle["challenger"], c_text, c_buttons, is_challenger=True
        )
        success_opponent = await send_or_edit_message(
            battle["opponent"], o_text, o_buttons, is_challenger=False
        )

        if not success_challenger or not success_opponent:
            print(f"UI update partially failed for battle {bid}")

    except Exception as e:
        print(f"Complete UI update failed for battle {bid}: {e}")
...
async def cb_move_select(event):
    # For doubles: battle:move_select:bid:side:slot:move
    bid = event.pattern_match.group(1).decode()
    side = event.pattern_match.group(2).decode()
    slot = int(event.pattern_match.group(3).decode())
    ...
    for i, target in enumerate(battle["active"][other_side]):
        if target["hp"] > 0:
            buttons.append([Button.inline(
                ...
                f"battle:move_target:{bid}:{side}:{slot}:{move_selected}:{i}"
            )])

    # Also allow targeting own Pokémon for some moves
    if battle["type"] == "doubles":
        text += "\n\nOr target ally:"
        for i, ally in enumerate(battle["active"][side]):
            if ally["hp"] > 0 and i != slot:
                buttons.append([Button.inline(
                    f"Ally {i+1}: {ally['name']} ({ally['hp']}/{ally['max_hp']} HP)",
                    f"battle:move_target:{bid}:{side}:{slot}:{move_selected}:{i}:ally"
                )])
...
async def cb_move_target(event):
    # battle:move_target:bid:side:slot:move:target_slot[:ally]
    bid = event.pattern_match.group(1).decode()
    side = event.pattern_match.group(2).decode()
    slot = int(event.pattern_match.group(3).decode())
    move_selected = event.pattern_match.group(4).decode()
