...
def create_battle(challenger_id, opponent_id, battle_type):
    ...
    battles[bid] = {
        ...
        "type": battle_type,
        "state": "pending",
        "pending_action": {  # FIXED: Use consistent naming
            "challenger": None,
            "opponent": None
        },
        "turn": 1
...
async def send_battle_ui(bid):
    ...
    c_buttons = []
    if not challenger_fainted and battle["state"] == "active":
    ...
    elif challenger_fainted:
        c_text += "\n\n❌ Your Pokémon fainted! Choose a replacement:"
        # Add switch options for non-fainted Pokémon
    ...
    o_buttons = []
    if not opponent_fainted and battle["state"] == "active":
...
async def process_action(battle, side, other_side, action):
    ...
    if action.startswith("switch:"):
        ...
        try:
            ...
            if new_pkm["hp"] <= 0:
                return f"{new_pkm['name']} has fainted and cannot battle!"
            
            battle["active"][side] = new_pkm
            return f"🔄 {new_pkm['name']} was switched in!"
        except (IndexError, ValueError, KeyError):
            return "❌ Switch failed - invalid Pokémon selection!"
    else:
        # Regular move
        try:
            ...
            if attacker["hp"] <= 0:
                return f"{attacker['name']} has fainted and cannot attack!"
            ...
            return f"{battle['active'][side]['name']} used {action}!\nBut something went wrong!"
...
async def resolve_turn(bid):
    ...
    # Get actions (FIXED: consistent naming)
    ...
    result1 = await process_action(battle, first, second, battle["pending_action"][first])
    results.append(result1)
    
    # Check if defender fainted before second move
    if battle["active"][second]["hp"] > 0:
        # Process second action
        result2 = await process_action(battle, second, first, battle["pending_action"][second])
        results.append(result2)

    # Send results to both players
    battle_text = "\n\n".join(results)
    try:
        await bot.send_message(battle["challenger"], f"📢 Turn Results:\n\n{battle_text}")
        await bot.send_message(battle["opponent"], f"📢 Turn Results:\n\n{battle_text}")
    except Exception as e:
        print(f"Failed to send turn results: {e}")
...
async def cb_move(event):
    ...
    # Save chosen move (FIXED: consistent naming)
...
async def cb_choose_switch(event):
    ...
    # Register switch action (FIXED: consistent naming)
