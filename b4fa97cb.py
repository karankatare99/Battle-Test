...
# -------------------------------
# FIXED BATTLE SYSTEM WITH FORCED SWITCH
# -------------------------------
...
async def resolve_turn(bid):
    ...
    # Check if defender fainted after first move - CRITICAL FIX
    if battle["active"][second]["hp"] <= 0:
        # Set forced switch flag for the fainted side
        battle["forced_switch"][second] = True
        results.append(f"{battle['active'][second]['name']} fainted and couldn't use its move!")
    else:
        # Process second action only if Pokemon didn't faint
