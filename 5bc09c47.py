...
async def send_summary(event, poke):
    text = (
        ...
        f"📊 EVs:\n"
        f"HP: {poke.get('ev_stats', {}).get('evhp', 0)} | Atk: {poke.get('ev_stats', {}).get('evatk', 0)} | Def: {poke.get('ev_stats', {}).get('evdef', 0)}\n"
        f"SpA: {poke.get('ev_stats', {}).get('evspa', 0)} | SpD: {poke.get('ev_stats', {}).get('evspd', 0)} | Spe: {poke.get('ev_stats', {}).get('evspe', 0)}\n\n"
        f"🧬 IVs:\n"
        f"HP: {poke.get('iv_stats', {}).get('ivhp', 31)} | Atk: {poke.get('iv_stats', {}).get('ivatk', 31)} | Def: {poke.get('iv_stats', {}).get('ivdef', 31)}\n"
        f"SpA: {poke.get('iv_stats', {}).get('ivspa', 31)} | SpD: {poke.get('iv_stats', {}).get('ivspd', 31)} | Spe: {poke.get('iv_stats', {}).get('ivspe', 31)}\n\n"
...
def build_perspective_texts(battle, attacker_side, defender_side, base_text, attacker_name, defender_name):
    ...
    if attacker_side == "challenger":
        # Challenger's Pokémon attacks opponent's Pokémon
        chal_attacker = challenger_name
        chal_defender = f"Opposing {opponent_name}"
    else:
        # Opponent's Pokémon attacks challenger's Pokémon
...
async def send_battle_ui(bid, battle_texts=None, buttons=None):
    ...
    elif battle["state"] == "active" and buttons:
    ...
    # Send or edit messages properly
    try:
        # For challenger
        if battle["message_ids"]["challenger"]:
            try:
                ...
                await bot.edit_message(
                    ...
                    c_text, 
                    buttons=c_buttons if buttons else None
                )
            except Exception as e:
                print(f"Edit failed for challenger: {e}")
                # Send new message if edit fails
                msg = await bot.send_message(battle["challenger"], c_text, buttons=c_buttons if buttons else None)
        ...
        if battle["message_ids"]["opponent"]:
            try:
                ...
                await bot.edit_message(
                    ...
                    o_text, 
                    buttons=o_buttons if buttons else None
                )
            except Exception as e:
                print(f"Edit failed for opponent: {e}")
                # Send new message if edit fails
                msg = await bot.send_message(battle["opponent"], o_text, buttons=o_buttons if buttons else None)
...
async def animate_battle_sequence(bid, actions_results):
    ...
    for i, (text_challenger, text_opponent, hp_changes) in enumerate(actions_results):
        ...
        await send_battle_ui(bid, battle_texts, False)
...
async def resolve_turn(bid):
    ...
    await send_battle_ui(bid, None, True)
