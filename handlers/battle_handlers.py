...
async def damage_calc_fn(level, power, attack, defense, type_eff_multiplier):
...
async def move_handler(user_id, move, poke, fmt, event):
    ...
    if fmt == "singles":
        try:
            ...
            user_movetext2 = f"It's {type_eff}!"
            opp_movetext1 = f"Opposing {self_pokemon} used {move}"
            opp_movetext2 = f"It's {type_eff}!"
...
async def battle_ui(mode,fmt,user_id,event):
    if fmt == "singles":
        ...
        for i in movetext[p1_id]["text_sequence"]:
            if i == movetext[p1_id]["text_sequence"][0]:
                try:
                    text = f"{i}\n\n{p1_text}"
            ...
            elif i == movetext[p1_id]["text_sequence"][-1]:
                try:
                    text = f"{i}\n\n{p1_text}"
                ...
                try:
                    ...
                    print(f"DEBUG: Error updating p1 battle UI: {e}")
        for i in movetext[p2_id]["text_sequence"]:
            if i == movetext[p2_id]["text_sequence"][0]:
                try:
                    text = f"{i}\n\n{p2_text}"
            ...
            elif i == movetext[p2_id]["text_sequence"][-1]:
                try:
                    text = f"{i}\n\n{p2_text}"
