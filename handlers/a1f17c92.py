...
def register_battle_handlers(bot):
    ...
    async def battle_stadium(event):
        ...
        if battle_state.get(user_id):
            await event.reply("This action can't be done")
            return
    ...
    async def select_format(event):
        ...
        # Consistent user_id type (always int)
        user_id = int(user_id)
        ...
        battle_state[user_id]["active_pokemon"] = []
        battle_state[user_id]["battle_started"] = False
        battle_state[user_id]["battle_initiated"] = True
    ...
    async def select_pokemon(event):
        ...
        p1p = []
        for i in battle_state[user_id]["team"]:
            element = i.split("_")[0]
    ...
    async def handle_move(event):
        ...
        try:
            ...
            await awaiting_move_action(room_id, fmt, move, poke, event)

        except KeyError as e:
            print(f"KeyError in handle_move: {e}")
            await event.answer("Battle state error occurred", alert=True)
...
def db_battle_extractor(user_id, mode, format):
    ...
    user_dict[int(user_id)]["mode"] = mode
    user_dict[int(user_id)]["fmt"] = format

    user_team = user_data["team"]
    user_dict[int(user_id)]["team"] = user_team
    ...
    user_dict[int(user_id)]["pokemon"] = user_poke
    return user_dict
    ...
    for i in range(0, len(team), 2):
        ...
        for j in range(2):
            if i + j < len(team):
                ...
                if user_id in selectteam and poke_data in selectteam[user_id]["pokes"]:
                    button_text = f"✅ {poke}"
                else:
                    button_text = f"⬜ {poke}"

                row.append(Button.inline(button_text, f"{user_id}:{mode}:{fmt}:select:{poke_data}"))
        buttons.append(row)

    # Add Done + View Opponent buttons at the bottom
    buttons.append([
        Button.inline("Done", f"{user_id}:{mode}:{fmt}:done"),
        Button.inline("View Opponent Team", f"{user_id}:{mode}:{fmt}:opp_team")
    ])
...
async def first_battle_ui(mode, fmt, user_id, event):
    ...
    try:
        if fmt == "singles":
            ...
            # Doubles battle implementation remains the same but with better error handling
            roomid = room[user_id]["roomid"]
            p1_id = int(room_userids[roomid]["p1"])
            p2_id = int(room_userids[roomid]["p2"])

            # Add similar implementation for doubles...
            print("Doubles battle UI not fully implemented yet")
        ...
        try:
            ...
            if selected_move.get(p1) and selected_move.get(p2):
                if selected_move[p1]["turn"] == battle_state[p1]["turn"] and selected_move[p2]["turn"] == battle_state[p2]["turn"]:
                    # Get active Pokemon correctly
                    p1_active = battle_state[p1]["active_pokemon"][0]
                    p2_active = battle_state[p2]["active_pokemon"][0]

                    p1_speed = battle_data[p1]["pokemon"][p1_active]["final_spe"]
                    p2_speed = battle_data[p2]["pokemon"][p2_active]["final_spe"]

                    if p1_speed > p2_speed:
                        await move_handler(p1, fmt, selected_move[p1]["move"], p1_active, event)
                        if battle_data[p2]["pokemon"][p2_active]["current_hp"] > 0:
                            await move_handler(p2, fmt, selected_move[p2]["move"], p2_active, event)

                    elif p2_speed > p1_speed:
                        await move_handler(p2, fmt, selected_move[p2]["move"], p2_active, event)
                        if battle_data[p1]["pokemon"][p1_active]["current_hp"] > 0:
                            await move_handler(p1, fmt, selected_move[p1]["move"], p1_active, event)

                    # Update battle UI after moves
                    await first_battle_ui(battle_state[p1]["mode"], fmt, p1, None)
                    return

        except KeyError as e:
            print(f"KeyError in awaiting_move_action: {e}")
            await event.edit("Battle state error occurred")
            return
...
async def standing_by_fn(event, user_id):
    ...
    while True:
        try:
            ...
            print(f"KeyError in standing_by_fn: {e}")
            await event.edit("Battle initialization error occurred")
            break
