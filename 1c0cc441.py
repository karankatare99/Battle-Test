...
def create_battle(challenger_id, opponent_id, battle_type):
    ...
    battle_storage[bid] = {
        ...
        "last_content": {"challenger": "", "opponent": ""}     # Track last message content to avoid duplicates
...
async def safe_edit_message(user_id, message_id, text, buttons=None):

    """Safely edit a message, handling 'Content not modified' errors."""
...
async def send_battle_ui(bid, battle_texts=None, buttons=None):
    ...
    # Update messages with duplicate content checking

    try:

        # Check for challenger
        ...
        if battle["message_ids"]["challenger"]:
            ...
            if buttons is True:
                ...
                if c_text != battle["last_content"]["challenger"]:

                    await safe_edit_message(battle["challenger"], battle["message_ids"]["challenger"], c_text, c_buttons)

                    battle["last_content"]["challenger"] = c_text

            else:

                # Animation updates - always edit

                await safe_edit_message(battle["challenger"], battle["message_ids"]["challenger"], c_text)

        else:

            if buttons is True and battle.get('turn', 1) == 1:
        ...
        if battle["message_ids"]["opponent"]:
            ...
            if buttons is True:
                ...
                if o_text != battle["last_content"]["opponent"]:

                    await safe_edit_message(battle["opponent"], battle["message_ids"]["opponent"], o_text, o_buttons)

                    battle["last_content"]["opponent"] = o_text

            else:

                # Animation updates - always edit

                await safe_edit_message(battle["opponent"], battle["message_ids"]["opponent"], o_text)
...
async def cb_target_select(event):
    ...
    if battle["type"] == "doubles":
        ...
        # Don't call send_battle_ui here to avoid duplicate content errors
