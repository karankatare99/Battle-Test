...
async def send_battle_ui(bid, battle_texts=None, buttons=None):
    ...
    if challenger_fainted or battle["forced_switch"]["challenger"]:
        ...
        if battle["type"] == "singles":
            ...
            # Check how many actions are still needed

            actions_needed = len([slot for slot in range(len(challenger_active)) 

                                if slot not in battle["pending_action"]["challenger"] 

                                and challenger_active[slot]["hp"] > 0])



            if actions_needed > 0:

                # Find the next slot that needs an action

                for slot in range(len(challenger_active)):

                    if (slot not in battle["pending_action"]["challenger"] and 

                        challenger_active[slot]["hp"] > 0):



                        c_text += f"\n\nChoose action for {challenger_active[slot]['name']} (Slot {slot+1}):"



                        # Move buttons with target selection needed

                        for move in challenger_active[slot].get("moves", []):

                            c_buttons.append([Button.inline(f"🎯 {move} (choose target)", f"battle:move_select:{bid}:challenger:{slot}:{move}")])



                        # Switch button

                        c_buttons.append([Button.inline("🔄 Switch", f"battle:switch:{bid}:challenger:{slot}")])
    ...
    # Build opponent UI (similar structure)
    ...
    if opponent_fainted or battle["forced_switch"]["opponent"]:
        ...
        if battle["type"] == "singles":
            ...
            actions_needed = len([slot for slot in range(len(opponent_active)) 

                                if slot not in battle["pending_action"]["opponent"] 

                                and opponent_active[slot]["hp"] > 0])
            ...
            if actions_needed > 0:
                ...
                for slot in range(len(opponent_active)):
                    ...
                    if (slot not in battle["pending_action"]["opponent"] and 

                        opponent_active[slot]["hp"] > 0):



                        o_text += f"\n\nChoose action for {opponent_active[slot]['name']} (Slot {slot+1}):"



                        # Move buttons with target selection needed

                        for move in opponent_active[slot].get("moves", []):

                            o_buttons.append([Button.inline(f"🎯 {move} (choose target)", f"battle:move_select:{bid}:opponent:{slot}:{move}")])



                        # Switch button

                        o_buttons.append([Button.inline("🔄 Switch", f"battle:switch:{bid}:opponent:{slot}")])
